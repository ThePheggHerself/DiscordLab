...
WriteToConsole(`Bot starting up. ${process.version}`, "info");


// #region Bot events
botClient.on("ready", () => {
	WriteToConsole(`Bot online and listening to port ` + config.port, "info");
...
tcpSocket.on("connection", (socket) => {
		...
		try {
			...
			array.forEach(json => {
				...
				if (object.Type === 0) {
					...
					WriteToConsole(`${object.CommandMessage}`, "info");
	...
	socket.on("close", () => {
		...
		WriteToConsole(`Socket closed!`, "info");
	...
	socket.on("error", error => {
		...
		WriteToConsole(`Socket closed: ${error}`, "info");
		botClient.user.setPresence({ game: { name: `for server startup`, type: "WATCHING" }, status: 'dnd' });
	});
	socket.on("timeout", timeout => {
		...
		WriteToConsole(`Socket closed: timeout!`, "info");
		botClient.user.setPresence({ game: { name: `for server startup`, type: "WATCHING" }, status: 'dnd' });
	});
});
...
async function WriteToConsole(message, type) {
	var date = new Date();
	console.debug(`[${date.toLocaleDateString()} ${date.toLocaleTimeString()}] [${type.toUpperCase()}] ${message}`);
}
